...
<div class="wcw-account-wrapper">
    <h3 class="wcw-main-title"><?php esc_html_e( 'Minha Carteira', 'woo-digital-wallet' ); ?></h3>
    <p class="wcw-subtitle"><?php esc_html_e( 'Acompanhe seu saldo e adicione créditos para suas próximas compras.', 'woo-digital-wallet' ); ?></p>

    <!-- Card de Saldo Proporcional -->
    <div class="wcw-balance-card <?php echo esc_attr( $balance_status_class ); ?>">
        <div class="wcw-balance-info">
            <span class="wcw-balance-label"><?php esc_html_e( 'Saldo Disponível', 'woo-digital-wallet' ); ?></span>
            ...
            <?php if ( ! empty( $pending_balance ) && $pending_balance > 0 ) : ?>
                ...
                <div class="wcw-pending-box">
                    <span class="wcw-pending-label"><?php esc_html_e( 'Aguardando Pagamento/Envio', 'woo-digital-wallet' ); ?></span>
                    <span class="wcw-pending-value">+ R$ <?php echo esc_html( $formatted_pending ); ?></span>
                </div>
            <?php endif; ?>

            <?php if ( $balance < 0 ) : ?>
                <span class="wcw-balance-warning"><?php esc_html_e( 'Saldo negativo devido a estorno recente.', 'woo-digital-wallet' ); ?></span>
            <?php endif; ?>
        </div>
       ...
       <!-- Formulário Compacto de Recarga (Condicional) -->
        <?php if ( 'yes' === get_option( 'wcw_wallet_recharge_status', 'yes' ) ) : ?>
            <div class="wcw-recharge-box">
                <span class="wcw-recharge-label"><?php esc_html_e( 'Adicionar créditos à carteira:', 'woo-digital-wallet' ); ?></span>
                <form method="POST" action="" class="wcw-recharge-form">
                    ...
                    <button type="submit" name="wcw_add_credito_submit" class="button wcw-btn-recharge">
                        <?php esc_html_e( 'Adicionar', 'woo-digital-wallet' ); ?>
                    </button>
                </form>
            </div>
        <?php endif; ?>
    ...
    <h4 class="wcw-table-title"><?php esc_html_e( 'Histórico de Transações', 'woo-digital-wallet' ); ?></h4>
    <div class="wcw-table-responsive">
        <table class="woocommerce-orders-table shop_table shop_table_responsive account-orders-table wcw-custom-table">
            <thead>
                <tr>
                    <th class="woocommerce-orders-table__header"><span><?php esc_html_e( 'Data', 'woo-digital-wallet' ); ?></span></th>
                    <th class="woocommerce-orders-table__header"><span><?php esc_html_e( 'Descrição', 'woo-digital-wallet' ); ?></span></th>
                    <th class="woocommerce-orders-table__header wcw-text-right"><span><?php esc_html_e( 'Valor', 'woo-digital-wallet' ); ?></span></th>
                </tr>
            </thead>
            <tbody>
                <?php if ( ! empty( $history ) ) : ?>
                    ...
                    ?>
                        <tr class="woocommerce-orders-table__row">
                            <td class="woocommerce-orders-table__cell" data-title="<?php esc_attr_e( 'Data', 'woo-digital-wallet' ); ?>">
                                <?php echo esc_html( $data_formatada ); ?>
                            </td>
                            <td class="woocommerce-orders-table__cell" data-title="<?php esc_attr_e( 'Descrição', 'woo-digital-wallet' ); ?>">
                                <?php echo wp_kses_post( $badge_html ) . esc_html( $txn['descricao'] ); ?>
                            </td>
                            <td class="woocommerce-orders-table__cell wcw-text-right wcw-font-bold <?php echo esc_attr( $txn_text_class ); ?>" data-title="<?php esc_attr_e( 'Valor', 'woo-digital-wallet' ); ?>">
                ...
                <?php else : ?>
                    <tr>
                        <td colspan="3" class="wcw-empty-ledger">
                            <em><?php esc_html_e( 'Nenhuma transação encontrada.', 'woo-digital-wallet' ); ?></em>
                        </td>
                    </tr>
                <?php endif; ?>
            </tbody>
